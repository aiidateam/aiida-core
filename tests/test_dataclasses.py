...
O 0.5 0.5 0.5
    ...
    @pytest.mark.skipif(not has_ase(), reason='Unable to import ase')
    @pytest.mark.skipif(not has_pycifrw(), reason='Unable to import PyCifRW')
    @pytest.mark.skipif(not has_pymatgen(), reason='Unable to import pymatgen')
    @pytest.mark.requires_rmq
...
_tag   {}
 '''.format('a' * 5000))
    ...
    @pytest.mark.skipif(not has_ase(), reason='Unable to import ase')
    @pytest.mark.skipif(not has_pycifrw(), reason='Unable to import PyCifRW')
    @pytest.mark.skipif(not has_spglib(), reason='Unable to import spglib')
    @pytest.mark.requires_rmq
...
class TestStructureData:
    ...
    def test_kind_8(self):
        ...
        sym.pop('translations')
        assert b.get_chemical_symbols() == ['C']
        assert numpy.round(b.cell, 2).tolist() == [[10, 0, 0], [-5, 8.66, 0], [0, 0, 10]]
...
class TestTrajectoryData:
    ...
    def test_conversion_to_structure(self):
        ...
        for struc in [from_step, from_get_structure]:
            ...
            assert round(abs(abs(numpy.array(struc.cell) - cells[1]).sum() - 0), 7) == 0
...
class TestKpointsData:
    ...
    def test_list(self):
        ...
        # try to get the same
        assert numpy.array_equal(input_klist, klist)
        ...
        assert numpy.array_equal(weights, input_weights)
        assert numpy.array_equal(klist, input_klist)
    ...
    def test_kpoints_to_cartesian(self):
        ...
        assert not numpy.array_equal(klist, input_klist)
        ...
        klist = k.get_kpoints(cartesian=True)
        assert numpy.allclose(klist, input_klist, atol=1e-16)
    ...
    def test_tetra_z_wrapper_legacy(self):
        ...
        assert round(abs(point_coords['Z'][2] - numpy.pi / alat), 7) == 0
...
@pytest.mark.usefixtures('clear_database_before_test_class')
class TestSeekpathExplicitPath:
...
class TestSeekpathPath:
    ...
    def test_simple(self):
        ...
        assert to_list_of_lists(retdict['path']) == [['GAMMA', 'X'], ['X', 'M'], ['M', 'GAMMA'], ['GAMMA', 'Z'],
                                                     ['Z', 'R'], ['R', 'A'], ['A', 'Z'], ['X', 'R'], ['M', 'A']]
...
class TestBandsData:
    ...
    def test_band(self):
        ...
        b = BandsData()
        b.set_kpointsdata(k)
        assert numpy.array_equal(b.cell, k.cell)
        ...
        assert numpy.array_equal(bands, input_bands)
        assert numpy.array_equal(occupations, input_occupations)
        assert b.units == 'ev'
