...
The simplest way to run these tests is to use `molecule` directly:

```console
$ pip install "molecule[docker]~=3.1.0" ansible~=2.10.0 docker~=4.2
$ export MOLECULE_GLOB=.molecule/*/config_local.yml
$ molecule test
```
...
```console
$ AIIDA_TEST_WORKERS=4 molecule test
```
