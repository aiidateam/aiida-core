...
class TestAuthinfo:
    ...
    def test_set_auth_params(self):
        ...
        try:
            ...
            self.auth_info.set_auth_params(auth_params)
            assert self.auth_info.get_auth_params()['password'] == authinfos.Password.REDACTED.value
    ...
    def test_delete_authinfo(self):
        ...
        assert BackendAuthInfo.SecureStorage(self.computer.uuid).get_password() is None

    def test_delete_authinfo_keeps_shared_password(self):
        """Deleting one authinfo must keep the password while other authinfos of the computer remain."""
