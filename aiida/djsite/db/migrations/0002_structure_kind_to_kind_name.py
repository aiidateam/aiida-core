# -*- coding: utf-8 -*-
"""
Change the internal name of the kind string without StructureData objects
from 'kind' to 'kind_name'
"""
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        attributes = orm.Attribute.objects.filter(
            dbnode__type="data.structure.StructureData.", key='_sites')
        for a in attributes:
            if a.datatype == 'json':
                sites = json.loads(a.tval)
                for site in sites:
                    if 'kind' in site:
                        kind_name = site['kind']
                        del site['kind']
                        site['kind_name'] = kind_name
                a.tval = json.dumps(sites)
                a.save()
