...
def test_load_node_class_legacy_code_raises():
    """Loading a legacy ``Code`` node type should raise until the storage is migrated."""
    with pytest.raises(exceptions.IncompatibleStorageSchema, match='not been migrated'):
        load_node_class('data.core.code.Code.')
