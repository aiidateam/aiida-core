...
def test_migrate_main_0000b_to_main_0001(tmp_path):
    ...
    with create_sqla_engine(database_path).connect() as connection:
        ...
        assert version == 'main_0001'

    with _alembic_connect(database_path) as config:
        downgrade(config, 'main_0000b')
    ...
    metadata = {**default_metadata, 'export_version': 'main_0000b'}

    with zipfile.ZipFile(input_path, 'w') as zf:
        zf.writestr(META_FILENAME, json.dumps(metadata))
        zf.write(db_path, DB_FILENAME)

    migrate(input_path, output_path, 'main_0001')

    assert SqliteZipBackend.get_current_archive_version(output_path) == 'main_0001'

    with zipfile.ZipFile(output_path) as zf:
        assert json.loads(zf.read(META_FILENAME))['export_version'] == 'main_0001'
