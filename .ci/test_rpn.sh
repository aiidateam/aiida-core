...
if [ -n "$EXPRESSIONS" ]; then
    for expression in "${EXPRESSIONS[@]}"; do
        coverage run -a $VERDI -p ${TEST_AIIDA_BACKEND} run "${CLI_SCRIPT}" -X $CODE -C -F -d -t $TIMEOUT "$expression"
    done
else
    for i in $(seq 1 $NUMBER_WORKCHAINS); do
        coverage run -a $VERDI -p ${TEST_AIIDA_BACKEND} run "${CLI_SCRIPT}" -X $CODE -C -F -d -t $TIMEOUT
    done
fi