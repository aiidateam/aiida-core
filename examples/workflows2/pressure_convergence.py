...
class PressureConvergence(FragmentedWorkfunction):
    ...
    def not_converged(self, ctx):
        return abs(ctx.r1['output_parameters'].dict.volume - ctx.r0['output_parameters'].dict.volume) > self.inputs.volume_tolerance


    def report(self, ctx):
        from aiida.orm import DataFactory
        self.out("steps", DataFactory('parameter')(dict={'steps': ctx.steps}))
...
if __name__ == "__main__":
    ...
    print run(PressureConvergence, structure=structure, code=Str(args.code), pseudo_family=Str(args.pseudo), volume_tolerance=Float(0.1))
