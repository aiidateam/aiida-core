...
def test_compress_error_handling(custom_transport, tmp_path_remote, monkeypatch):
...
def test_compress_basic(
    custom_transport, format, dereference, create_file_hierarchy, file_hierarchy, tmp_path_remote, tmp_path_local
):
    ...
    with custom_transport as transport:
        ...
        Path(tmp_path_local / 'extracted' / 'file.txt').read_text() == 'file'
        Path(tmp_path_local / 'extracted' / 'folder' / 'file_1').read_text() == '1'

        # test the symlink
        if dereference:
            assert not os.path.islink(tmp_path_local / 'extracted' / 'symlink')
            Path(tmp_path_local / 'extracted' / 'symlink').read_text() == 'file'
        else:
...
def test_extract(
    custom_transport, create_file_hierarchy, file_hierarchy, format, tmp_path_remote, tmp_path_local, monkeypatch
):
    ...
    Path(tmp_path_local / 'extracted' / 'file.txt').read_text() == 'file'
    Path(tmp_path_local / 'extracted' / 'folder_1' / 'file_1').read_text() == '1'
