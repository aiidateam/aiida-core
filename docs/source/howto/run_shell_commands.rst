...
.. note::
    ...
    Identifying such a parser means serializing everything it captured, and a node refuses to be pickled, so a
    parser that refers to one is refused at submission.
    Capture the pk and load the node inside the parser instead::
