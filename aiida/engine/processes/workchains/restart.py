...
class BaseRestartWorkChain(WorkChain):
    ...
    errors through error handlers that can be attached dynamically through the `register_process_handler` decorator.
    ...
    def inspect_process(self):  # pylint: disable=inconsistent-return-statements,too-many-branches
        ...
        If the process is excepted or killed, the workchain will abort. Otherwise any attached handlers will be called
        ...
        happens twice in a row, the workchain is aborted. In the case that at least one handler returned a report the
        ...
        and it will move on to the next step.
        """
    ...
    def _handlers(cls):  # pylint: disable=no-self-argument
        """Return the tuple of all registered handlers.
