...
then
    ...
    then
        ...
        verdi -p $TEST_AIIDA_BACKEND computer configure torquessh --non-interactive --username=app --port=10022 --key-file=~/.ssh/id_rsa --timeout=60 --compress --gss-host=localhost --load-system-host-keys --key-policy=RejectPolicy
        ...
        verdi -p $TEST_AIIDA_BACKEND computer configure torquessh --non-interactive --label=jenkins --port=22 --key-file=~/.ssh/id_rsa --timeout=60 --compress --gss-host=localhost --load-system-host-keys --key-policy=RejectPolicy
