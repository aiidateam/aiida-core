...
The export will by default constitute a complete graph and all files associated with the nodes will be added to the export archive alongside the provenance graph.
...
See ``verdi export create --help`` for a full list of available options.

* **Selection:** Nodes can be selected via identifier (PK, UUID, label).
  It is often convenient to first put the nodes you would like to export into a :py:class:`~aiida.orm.groups.Group` and then to export the group using ``verdi export create -G <group-identifier>``.
...
* **Compression:** By default, the export file is a compressed ``zip`` archive.
  Use the command line options to disable compression or to switch to the gzipped tar ball format.
...
See ``verdi import --help`` for a full list of available options.

* **Duplication:** AiiDA will avoid identifier collisions and node duplication based on UUIDs (or email comparisons for user entries).
...
Furthermore, using the ``--version`` flag, you can quickly inspect the archive's export version.
...
Old AiiDA archives may be migrated to the newest version using ``verdi export migrate``, in order to make them importable with the newest AiiDA version.

.. warning::

    It is not possible to "downgrade" an archive to previous export versions.
    However, by default the migration will produce a new file.
    So you will have your archive in at least two different export versions.
...
As a default, ``verdi import`` will call ``verdi export migrate`` to try and migrate the archive if an old archive is supplied.
It will always ask prior to migrating, unless the ``-n/--non-interactive`` option is used.
One can also avoid automatic migration by passing ``--no-migration`` to ``verdi import``.

The archives are by default not migrated in place, and will indeed never be migrated in place when migrated through calls to ``verdi import``.
Instead ``verdi export migrate`` expects an output filename and will create a new archive using this filename.
If you wish to migrate an archive in place, please use ``verdi export migrate --in-place <archive>``.
Migrating in place may be a good solution for you, if the archive is very big or you do not have a lot of disk space.
