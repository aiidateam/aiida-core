...
def group_addnodes(group, force, nodes):
    ...
    if not group:
        echo.echo_critical("Please sepcify the group")
...
@click.argument('source_group_label', nargs=1, type=click.STRING)
@click.argument('destination_group_label', nargs=1, type=click.STRING)
@with_dbenv()
def group_copy(source_group_label, destination_group_label):
    """Add all nodes that belong to source group to the destination group (which may or may not exist)"""
    from aiida import orm

    try:
        src_group = orm.Group.objects.get(label=source_group_label)
    except NotExistent:
        echo.echo_critical("You cannot copy nodes from a group <{}> that does not exist".format(source_group_label))
    dest_group = orm.Group.objects.get_or_create(label=destination_group_label)[0]

    try:
        dest_group.add_nodes(src_group.nodes)
        echo.echo_success("Nodes were succesfully copied from the group <{}> to the group "
                          "<{}>".format(source_group_label, destination_group_label))
    except:
        raise
