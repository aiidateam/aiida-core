...
Certain subclasses of nodes need to adapt this behavior however, as for example in the case of the :py:class:`~aiida.orm.implementation.general.calculation.Calculation` class (see `calculation updatable attributes`_), but since the immutability
...
********************
The :py:class:`~aiida.orm.implementation.general.calculation.Calculation` class is a subclass of the :py:class:`~aiida.orm.implementation.general.node.AbstractNode` class, which means that its attributes become immutable once stored.
