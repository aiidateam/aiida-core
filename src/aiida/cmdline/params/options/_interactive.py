...
from ._conditional import ConditionalOption
...
class InteractiveOption(ConditionalOption):
    ...
    .. note:: This class has a parameter ``required_fn`` that can be passed to its ``__init__`` (inherited
        from the superclass :py:class:`~aiida.cmdline.params.options._conditional.ConditionalOption`) and a
        ``prompt_fn``.
        ...
        - ``prompt_fn`` is about "should I prompt for this value if in interactive mode" and only makes sense
          in this class and not in :py:class:`~aiida.cmdline.params.options._conditional.ConditionalOption`.
...
class TemplateInteractiveOption(InteractiveOption):
    ...
    def __init__(self, param_decls: Sequence[str] | None = None, **kwargs: t.Any):
        ...
        :param template: name of the template to use from the ``aiida.cmdline._templates`` directory.
    ...
    def prompt_for_value(self, ctx: click.Context) -> t.Any:
        ...
        from aiida.cmdline.utils._multi_line_input import edit_multiline_template
