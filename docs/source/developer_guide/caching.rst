...
* The ``_cacheable`` property is set to ``False`` in the :class:`aiida.orm.implementation.general.calculation.Calculation`, and only re-enabled in :class:`aiida.orm.implementation.general.calculation.job.JobCalculation` and :class:`CalcFunctionNode <aiida.orm.node.process.calculation.calcfunction.CalcFunctionNode>`. This means that a ``WorkflowNode`` will not be cached.
