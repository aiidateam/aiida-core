...
The ``core.ssh`` transport, which can be used for machines that can be connected to over ssh, is built on top of the `asyncssh <https://asyncssh.readthedocs.io/>`_ library that is installed as a required dependency of ``aiida-core``.
