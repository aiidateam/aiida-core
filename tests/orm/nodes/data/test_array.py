...
def test_get_array():
    ...
    node = ArrayData()
    with pytest.raises(ValueError, match=r'`name` not specified but the node contains no arrays.'):
        node.get_array()

    node = ArrayData({'a': numpy.array([]), 'b': numpy.array([])})
    with pytest.raises(ValueError, match=r'`name` not specified but the node contains multiple arrays.'):
        node.get_array()
