...
The :py:class:`~aiida.calculations._arithmetic.add.ArithmeticAddCalculation` is a calculation job implementation that forms an interface to accomplish exactly that for the example bash script.
...
These code nodes are instances of the :py:class:`~aiida.orm.nodes.data.code.abstract.AbstractCode` class, which is a sub-class of :py:class:`~aiida.orm.nodes.data.data.Data`.
