...
_IO_CALC_NODE_CONTENT_FLAT = [
    ...
    'file.txt',  # From inputs repo, node_inputs/singlefile,
    # node_inputs/folderdata, node_outputs/singlefile,
    # node_outputs/folderdata (potentially overwritten if names clash)
...
def get_expected_multiply_add_wc_tree(wc_pk: int, child_pks: Tuple[int, int]) -> Dict[str, List[Any]]:
    ...
    multiply_dir_key = list(multiply_child_tree.keys())[0]  # noqa: RUF015
    add_dir_key = list(add_child_tree.keys())[0]  # noqa: RUF015
...
def _assemble_nodes_by_type(node_trees: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    for node_tree in node_trees:
        node_key = list(node_tree.keys())[0]  # noqa: RUF015
        # TODO: Fix here
