...
def test_constructor():
    ...
    node = PickledData.from_object(None)
...
def test_load(obj):
    ...
    node = PickledData.from_object(obj)
...
def test_kwargs():
    ...
    pickled = PickledData.from_object(Node, recurse=True).store()
    ...
    with pytest.raises(TypeError, match="got an unexpected keyword argument 'unsupported_kwarg'"):
        pickled = PickledData.from_object(Node, unsupported_kwarg=True).store()
