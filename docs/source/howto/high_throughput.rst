...
.. _how-to:high-throughput:configuration:

Running high-throughput workflows in AiiDA requires attention to performance and resource balancing across the daemon, database, message broker, and storage.
This guide collects practical tuning tips and pointers for diagnosing bottlenecks.
For more background, see the :ref:`topic on performance<topics:performance>`.
