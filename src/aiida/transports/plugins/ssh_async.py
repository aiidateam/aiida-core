...
class AsyncSshTransport(AsyncTransport):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.pop('multiplexing', False):
            ...
            self.async_backend = AsyncSSHwrapper(self.machine, self.logger, self._bash_command_str)
    ...
    async def getfile_async(
        ...
        **kwargs,
        ...
        try:
            await self._lock()
            await self.async_backend.get(remotepath = remotepath,
                                        localpath = localpath,
        ...
        **kwargs,
        ...
        for content_ in content_list:
            try:
                ...
                await self.async_backend.get(remotepath = parentpath,
                                            localpath = localpath,
                                            dereference = dereference,
                                            preserve = preserve,
                                            recursive = True)
    ...
    async def putfile_async(
        ...
        **kwargs,
        ...
        try:
            await self._lock()
            await self.async_backend.put(localpath=localpath,
        ...
        **kwargs,
        ...
        for content_ in content_list:
            try:
                ...
                await self.async_backend.put(localpath=parentpath,
                                             remotepath=remotepath,
                                             dereference=dereference,
                                             preserve=preserve,
                                             recursive=True)
    ...
    async def copy_async(
        ...
        await self.async_backend.copy(remotesource=remotesource,
    ...
    async def exec_command_wait_async(
        ...
        **kwargs,
        ...
        result = await self.async_backend.run(command= command,
                                                stdin= stdin,
                                                timeout= timeout,)
    ...
    async def get_attribute_async(self, path: TransportPath):
        ...
        * st_mtime: time of most recent modification
        ...
        asyncssh_attr = await self.async_backend.lstat(path)
    ...
    async def chown_async(self, path: TransportPath, uid: int, gid: int):
        ...
        if self.path_exists(path):
            await self.async_backend.chown(path, uid, gid, follow_symlinks=True)
        else:
            raise OSError(f'Error, path {path} does not exist')
