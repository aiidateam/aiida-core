...
class _UnreachableStorage(ConfigurationError):  # noqa: N818
    """Raised when a connection to the storage backend fails."""


class _IncompatibleDatabaseSchema(ConfigurationError):  # noqa: N818
...
class _IncompatibleExternalDependencies(ConfigurationError):  # noqa: N818
...
class IncompatibleStorageSchema(_IncompatibleDatabaseSchema):
    """Raised when the storage schema is incompatible with that of the code."""
...
class _DatabaseMigrationError(AiidaException):
...
class StorageMigrationError(_DatabaseMigrationError):
    """Raised if a critical error is encountered during a storage migration."""
...
class _StashingError(AiidaException):
...
class _CircusCallError(AiidaException):
