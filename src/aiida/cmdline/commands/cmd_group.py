...
def group_delete(
    ...
    **traversal_rules,
    ...
    """Delete groups and (optionally) the nodes it contains."""
    from aiida import orm
    from aiida.tools import delete_group_nodes

    if groups and (all_users or user or type_string or past_days or startswith or endswith or contains or node):
...
@options.NODE(help='Show only the groups that contain a node.')
@with_dbenv()
def group_list(
    all_users,
    user,
