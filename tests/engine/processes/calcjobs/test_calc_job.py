...
class TestCalcJob:
    ...
    def test_run_local_code(self):
        ...
        for filename in self.local_code.ctx.repository.list_object_names():
    ...
    def test_provenance_exclude_list(self):
        ...
        assert 'base' in node.ctx.repository.list_object_names()
        assert sorted(['b']) == sorted(node.ctx.repository.list_object_names(os.path.join('base')))
        assert ['two'] == node.ctx.repository.list_object_names(os.path.join('base', 'b'))
...
def test_parse_not_implemented(generate_process):
    ...
    retrieved.ctx.repository.put_object_from_filelike(io.BytesIO(b'\n'), filename_stderr)
    retrieved.ctx.repository.put_object_from_filelike(io.BytesIO(b'\n'), filename_stdout)
    retrieved.store()
