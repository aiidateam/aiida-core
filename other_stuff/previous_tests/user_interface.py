'''
This file contains specifications on the user interface for object operations, retrieval and queries.
...
def SimpleWorkflow():
    ...
    #select code
    mycode = Code.find(name='Espresso-4.2')   #gets code object
    
    #select structure
    mystruc = Struc.find() #gets struc object    
    print mystruc.atoms
    
    #set params for job execution
    myjob = Job.create()
    myjob.params = {'NPROC' : 32, 'NPOOL' : 1, 'CALCTYPE' : 'md', 'RUNTIME' : '1:00:00'}
    # params are validated based on 

    
    #select which params are 
    
    #initiate and submit the calculation
...
s1 = Struc.objects.get_or_create(title='s1', formula='H2O', user=u)[0]
s2 = Struc.objects.get_or_create(title='s2', formula='CH4', user=u[0]
s3 = Struc.objects.get_or_create(title='s3', formula='K', user=u, type=st2)[0]
s4 = Struc.objects.get_or_create(title='s4', formula='NaCl', user=u, type=st2)[0]
