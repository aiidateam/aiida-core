...
class TestVerdiNode:
    ...
    def init_db(self, clear_database_before_test, run_cli_command):
...
class TestVerdiGraph:
    ...
    @pytest.fixture(autouse=True)
    def init_db(self, clear_database_before_test, tmp_path, run_cli_command):
        """Setup the database before each test"""
    ...
    def test_generate_graph(self):
        ...
        options = [root_node]
        try:
            result = self.cli_runner(cmd_node.graph_generate, options)
            assert result.exception is None, result.output
    ...
    def test_check_recursion_flags(self):
        ...
        for flag in ['-a', '--ancestor-depth', '-d', '--descendant-depth']:
            for badvalue in ['xyz', '3.14', '-5']:
                ...
                try:
                    result = self.cli_runner(cmd_node.graph_generate, options, raises=True)
                    assert not os.path.isfile(filename)
                finally:
                    delete_temporary_file(filename)
...
class TestVerdiRehash:
    ...
    def test_rehash_interactive_yes(self):
        ...
        result = self.cli_runner(cmd_node.rehash, options, input='y')
    ...
    def test_rehash_interactive_no(self):
        ...
        options = []  # no option, will ask in the prompt
        result = self.cli_runner(cmd_node.rehash, options, raises=True, input='n')
