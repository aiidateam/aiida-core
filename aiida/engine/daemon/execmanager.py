...
def upload_calculation(
    ...
    for code in input_codes:
        if code.is_local():
            ...
            for filename in code.ctx.repository.list_object_names():
                ...
                with NamedTemporaryFile(mode='wb+') as handle:
                    handle.write(code.ctx.repository.get_object_content(filename, mode='rb'))
                    handle.flush()
    ...
    for uuid, filename, target in local_copy_list:
        ...
        if data_node is None:
            ...
            if data_node.ctx.repository.get_object(filename_source).file_type == FileType.DIRECTORY:
                # If the source object is a directory, we copy its entire contents
                data_node.ctx.repository.copy_tree(filepath_target, filename_source)
                provenance_exclude_list.extend(data_node.ctx.repository.list_object_names(filename_source))
            else:
                # Otherwise, simply copy the file
                with folder.open(target, 'wb') as handle:
                    with data_node.ctx.repository.open(filename, 'rb') as source:
        ...
        for filename in filenames:
            ...
            if relpath not in provenance_exclude_list and all(
                ...
                with open(filepath, 'rb') as handle:  # type: ignore[assignment]
                    node.ctx.repository._repository.put_object_from_filelike(handle, relpath)  # pylint: disable=protected-access
    ...
    node.ctx.repository._update_repository_metadata()  # pylint: disable=protected-access
...
def retrieve_calculation(calculation: CalcJobNode, transport: Transport, retrieved_temporary_folder: str) -> None:
    ...
    with transport:
        ...
        with SandboxFolder() as folder:
            ...
            retrieved_files.ctx.repository.put_object_from_tree(folder.abspath)
