...
class TestNodeHashing:
    ...
    def create_folderdata_with_empty_file():
        ...
        with tempfile.NamedTemporaryFile() as handle:
            node.ctx.repository.put_object_from_filelike(handle, 'path/name')
        return node
    ...
    def create_folderdata_with_empty_folder():
        ...
        node = orm.FolderData()
        node.ctx.repository.put_object_from_tree(dirpath, 'path/name')
        return node
    ...
    def test_folder_file_different(self):
        ...
        assert f1.ctx.repository.list_object_names('path') == f2.ctx.repository.list_object_names('path')
...
class TestNodeBasic:
    ...
    def test_files(self):
        ...
        with tempfile.NamedTemporaryFile('w+') as handle:
            ...
            handle.flush()
            a.ctx.repository.put_object_from_file(handle.name, 'file1.txt')
            a.ctx.repository.put_object_from_file(handle.name, 'file2.txt')

        assert set(a.ctx.repository.list_object_names()) == set(['file1.txt', 'file2.txt'])
        with a.ctx.repository.open('file1.txt') as fhandle:
            assert fhandle.read() == file_content
        with a.ctx.repository.open('file2.txt') as fhandle:
            assert fhandle.read() == file_content
        ...
        assert set(b.ctx.repository.list_object_names()) == set(['file1.txt', 'file2.txt'])
        with b.ctx.repository.open('file1.txt') as handle:
            assert handle.read() == file_content
        with b.ctx.repository.open('file2.txt') as handle:
            assert handle.read() == file_content
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as handle:
            ...
            handle.flush()
            b.ctx.repository.put_object_from_file(handle.name, 'file2.txt')
            b.ctx.repository.put_object_from_file(handle.name, 'file3.txt')

        # I check the new content, and that the old one has not changed
        assert set(a.ctx.repository.list_object_names()) == set(['file1.txt', 'file2.txt'])
        with a.ctx.repository.open('file1.txt') as handle:
            assert handle.read() == file_content
        with a.ctx.repository.open('file2.txt') as handle:
            assert handle.read() == file_content
        assert set(b.ctx.repository.list_object_names()) == set(['file1.txt', 'file2.txt', 'file3.txt'])
        with b.ctx.repository.open('file1.txt') as handle:
            assert handle.read() == file_content
        with b.ctx.repository.open('file2.txt') as handle:
            assert handle.read() == file_content_different
        with b.ctx.repository.open('file3.txt') as handle:
            assert handle.read() == file_content_different
        ...
        with tempfile.NamedTemporaryFile(mode='w+') as handle:
            handle.write(file_content_different)
            handle.flush()
            c.ctx.repository.put_object_from_file(handle.name, 'file1.txt')
            c.ctx.repository.put_object_from_file(handle.name, 'file4.txt')
        ...
        assert set(c.ctx.repository.list_object_names()) == set(['file1.txt', 'file2.txt', 'file4.txt'])
        with c.ctx.repository.open('file1.txt') as handle:
            assert handle.read() == file_content_different
        with c.ctx.repository.open('file2.txt') as handle:
            assert handle.read() == file_content
        with c.ctx.repository.open('file4.txt') as handle:
            assert handle.read() == file_content_different
    ...
    def test_folders(self):
        ...
        a.ctx.repository.put_object_from_tree(tree_1, 'tree_1')

        # verify if the node has the structure I expect
        assert set(a.ctx.repository.list_object_names()) == set(['tree_1'])
        assert set(a.ctx.repository.list_object_names('tree_1')) == set(['file1.txt', 'dir1'])
        assert set(a.ctx.repository.list_object_names(os.path.join('tree_1', 'dir1'))) == set(['dir2', 'file2.txt'])
        with a.ctx.repository.open(os.path.join('tree_1', 'file1.txt')) as fhandle:
            assert fhandle.read() == file_content
        with a.ctx.repository.open(os.path.join('tree_1', 'dir1', 'file2.txt')) as fhandle:
            assert fhandle.read() == file_content

        # try to exit from the folder
        with pytest.raises(FileNotFoundError):
            a.ctx.repository.list_object_names('..')
        ...
        # Check that the content is there
        assert set(b.ctx.repository.list_object_names()) == set(['tree_1'])
        assert set(b.ctx.repository.list_object_names('tree_1')) == set(['file1.txt', 'dir1'])
        assert set(b.ctx.repository.list_object_names(os.path.join('tree_1', 'dir1'))) == set(['dir2', 'file2.txt'])
        with b.ctx.repository.open(os.path.join('tree_1', 'file1.txt')) as fhandle:
            assert fhandle.read() == file_content
        with b.ctx.repository.open(os.path.join('tree_1', 'dir1', 'file2.txt')) as fhandle:
            assert fhandle.read() == file_content
        ...
        b.ctx.repository.put_object_from_tree(dir3, os.path.join('tree_1', 'dir3'))
        # no absolute path here
        with pytest.raises(TypeError):
            b.ctx.repository.put_object_from_tree('dir3', os.path.join('tree_1', 'dir3'))

        stream = io.StringIO(file_content_different)
        b.ctx.repository.put_object_from_filelike(stream, 'file3.txt')
        ...
        with a.ctx.repository.open(os.path.join('tree_1', 'dir1', 'file2.txt')) as fhandle:
            assert fhandle.read() == file_content
        # new
        assert set(b.ctx.repository.list_object_names()) == set(['tree_1', 'file3.txt'])
        assert set(b.ctx.repository.list_object_names('tree_1')) == set(['file1.txt', 'dir1', 'dir3'])
        assert set(b.ctx.repository.list_object_names(os.path.join('tree_1', 'dir1'))) == set(['dir2', 'file2.txt'])
        with b.ctx.repository.open(os.path.join('tree_1', 'file1.txt')) as fhandle:
            assert fhandle.read() == file_content
        with b.ctx.repository.open(os.path.join('tree_1', 'dir1', 'file2.txt')) as fhandle:
        ...
        # in the copy only
        stream = io.StringIO(file_content_different)
        c.ctx.repository.put_object_from_filelike(stream, os.path.join('tree_1', 'file1.txt'))
        c.ctx.repository.put_object_from_filelike(stream, os.path.join('tree_1', 'dir1', 'file4.txt'))
        c.ctx.repository.delete_object(os.path.join('tree_1', 'dir1', 'dir2'))
        ...
        # check new
        assert set(c.ctx.repository.list_object_names()) == set(['tree_1'])
        assert set(c.ctx.repository.list_object_names('tree_1')) == set(['file1.txt', 'dir1'])
        assert set(c.ctx.repository.list_object_names(os.path.join('tree_1',
                                                                   'dir1'))) == set(['file2.txt', 'file4.txt'])
        with c.ctx.repository.open(os.path.join('tree_1', 'file1.txt')) as fhandle:
            assert fhandle.read() == file_content_different
        with c.ctx.repository.open(os.path.join('tree_1', 'dir1', 'file2.txt')) as fhandle:
            assert fhandle.read() == file_content
