...
class WorkflowProcess(Process, metaclass=Protect):
    ...
    (which walks a static outline) and WorkGraph (which schedules by a graph of data dependencies). It owns
    everything the two have in common, the stepper seam, the awaitable-based waiting on child processes, the
    context (:attr:`ctx`), and the step lifecycle with its checkpointing, and leaves only the concrete stepping
    strategy to the subclass via :meth:`_create_stepper` / :meth:`_recreate_stepper`.

    Making WorkChain and WorkGraph siblings under this base, rather than having WorkGraph inherit from WorkChain,
    keeps the two execution models independent: neither carries the other's concepts, and a change to one
    strategy (the outline or the DAG) cannot affect the other.
    """
    ...
    def __init__(
        ...
        """Construct a stepper process instance.
    ...
    def _create_stepper(self) -> Stepper:
        """Return the stepper that drives this process. Subclasses must implement this.

        The stepper is the execution strategy: :class:`~aiida.engine.WorkChain` returns one that walks the outline
        declared on its spec, WorkGraph one that derives the order from a graph of data dependencies. A subclass
        that overrides this must also override :meth:`_recreate_stepper`, otherwise its processes cannot be restored
        from a checkpoint.
        """
        raise NotImplementedError

    def _recreate_stepper(self, saved_state: t.Any) -> Stepper:
        """Restore the stepper from the state it wrote to the checkpoint. Subclasses must implement this.
    ...
    def on_run(self):
        super().on_run()
        # ``set_stepper_state_info`` is part of the node contract for a stepper-driven process; both WorkChainNode
        # and WorkGraphNode provide it (the latter subclasses the former).
