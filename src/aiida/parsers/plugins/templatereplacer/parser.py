...
class TemplatereplacerParser(Parser):
    ...
    def parse(self, **kwargs):
        ...
        if retrieve_temporary_files is not None:
            ...
            for retrieved_file in retrieve_temporary_files:
                ...
                if not os.path.isfile(file_path):
                    self.logger.error(
                        f'the file {retrieved_file} was not found '
                        f'in the temporary retrieved folder {retrieved_temporary_folder}'
                    )
        ...
        label = self.process_class.spec().default_output_node  # type: ignore[attr-defined]
