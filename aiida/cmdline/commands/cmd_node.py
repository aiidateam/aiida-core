...
def repo_cat(node, relative_path):
    ...
    try:
        with node.ctx.repository.open(relative_path, mode='rb') as fhandle:
...
def repo_dump(node, output_directory):
    ...
    def _copy_tree(key, output_dir):  # pylint: disable=too-many-branches
        ...
        the ``output_dir``.
        """
        for file in node.ctx.repository.list_objects(key):
            ...
            if file.file_type == FileType.DIRECTORY:
                ...
                with node.ctx.repository.open(file_key, 'rb') as in_file:
