...
* Developer install tipps :ref:`install.other.devel`
...
The install procedure recommended for developers is as described in `install.quickstart` and `install.other.install`. 
The only difference is that you will want to switch to the development branch of the AiiDA repository before running `pip install -e`.
Example::
...
The same applies for feature branches.
