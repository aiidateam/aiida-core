...
from aida.common.exceptions import (InternalError, ModificationNotAllowed, NotExistent, ValidationError, AidaException )
...
class Workflow(object):
	...
	_logger       = aida.common.aidalogger.getChild('workflow')
	...
	def __init__(self,**kwargs):
			...
			if uuid is not None:
				...
				if caller_module == None or not caller_module.__name__.startswith("aida.workflows"):
	...
	def add_calculation(self, calc):
		...
		from aida.djsite.db import tasks

		import inspect

		if (not isinstance(calc,Calculation)):
			raise AidaException("Cannot add a calculation not of type Calculation")    					
	...
	def next(self, method):
		...
		import inspect
		from aida.common.datastructures import wf_start_call, wf_states, wf_exit_call
	...
	def get_calculations(self, method):
		...
		try:
			...
			raise AidaException("Cannot retrive step's calculations")
