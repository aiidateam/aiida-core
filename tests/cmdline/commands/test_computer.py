...
class TestComputerSetupTemplateConfig:
    ...
    def test_setup_template_missing_vars_raises(self, run_cli_command, template_config, config_first):
        """Non-interactive mode without ``--template-vars`` raises rather than prompting, in either option order."""
        config_args = ['--config', str(template_config)]
        args = config_args + ['--non-interactive'] if config_first else ['--non-interactive'] + config_args

        result = run_cli_command(computer_setup, args, raises=True)
