...
def test_parser_invalid_signature_keyword_only_parser(generate_shell_calc_job, generate_shell_code):
    """Test that a hook whose `parser` can only be passed by keyword is refused.

    The hook is called positionally, and `ShellParser.call_parser_hook` decides to pass the parser from the names in
    the signature, so a keyword-only one is detected and then cannot be supplied.
    """

    def keyword_only_parser(dirpath, *, parser):
        return {}

    with pytest.raises(ValueError, match=r'invalid function signature'):
        generate_shell_calc_job('core.shell', inputs={'code': generate_shell_code(), 'parser': keyword_only_parser})


def test_parser_invalid_signature_reversed(generate_shell_calc_job, generate_shell_code):
    """Test that a parser declaring its parameters the other way round is refused at submission.

    The hook is called positionally, so the two arguments would bind the wrong way and the job would fail after the
    command had already run, somewhere inside the parser rather than here.
    """

    def reversed_order(parser, dirpath):
        return {}

    with pytest.raises(ValueError, match=r'The `parser` has an invalid function signature'):
        generate_shell_calc_job('core.shell', inputs={'code': generate_shell_code(), 'parser': reversed_order})
...
def test_parser_over_daemon(generate_shell_code, submit_and_await):
    """Test submitting a ``ShellJob`` with a custom parser over the daemon, and what the finished job leaves behind.

    The record is asserted against a job that actually ran, because only a terminated process can show that the
    payload which carried the callable is gone with it, and that the repository holds source text and not a pickle.
    """
    value = 'testing'
