...
###########################################################################
"""An async queue for getting compute client instances from authinfo.

This queue allows one to register an interest in a client object,
which will be provided at some point in the future.
It is necessary in order to rate limit the opening of clients.
"""
import asyncio
import contextlib
import contextvars
...
class ClientRequest:
    """Information kept about request for a compute client."""
...
class ComputeClientQueue:
    """
    A queue to get compute client instances from authinfo.
    ...
    def loop(self) -> asyncio.AbstractEventLoop:
        """ Get the loop being used by this client queue """
        return self._loop
    ...
    def request_client(self, authinfo: AuthInfo) -> Iterator[Awaitable[ComputeClientProtocol]]:
        ...
        they will instead be given back a future that can be awaited to get the client::
        ...
        if client_request is None:
            # There is no existing request for this client (i.e. on this authinfo)
        ...
        try:
            ...
            _LOGGER.debug('Compute client task cancelled')
            raise
        except Exception:
            _LOGGER.error('Exception whilst using compute client:\n%s', traceback.format_exc())
            raise
        finally:
            client_request.count -= 1
            assert client_request.count >= 0, 'Compute client request count dropped below 0!'
            # Check if there are no longer any users that want the client
            if client_request.count == 0:
                if client_request.future.done():
                    _LOGGER.debug('Compute client request closing client for %s', authinfo)
