...
def aiida_computer_ssh(aiida_computer) -> t.Callable[[], Computer]:
    ...
    def factory(label: str | None = None, configure: bool = True, backend: str = 'asyncssh') -> Computer:
        """Create or load a computer with the ``core.ssh`` transport.

        :param label: The computer label. If not specified, a random UUID4 is used.
        :param configure: Boolean, if ``True``, ensures the computer is configured.
        :param backend: The backend of the asynchronous SSH transport, ``asyncssh`` or ``openssh``. Defaults to the
            one the plugin itself defaults to. Pass ``configure=False`` to leave a configured computer as it is.
        """
