...
class RESTApiTestSuite(RESTApiTestCase):
    ...
    ############### schema #############
    def test_computer_schema(self):
        """
        test computer schema
        """
        url = self.get_url_prefix() + '/computers/schema'
        with self.app.test_client() as client:
            rv = client.get(url)
            response = json.loads(rv.data)
            ...
            self.assertEqual(response["data"]["fields"], expected_schema_fields)
            self.assertEqual(response["data"]["ordering"], expected_schema_order)

    def test_user_schema(self):
        """
        test user schema
        """
        url = self.get_url_prefix() + '/users/schema'
        with self.app.test_client() as client:
            rv = client.get(url)
            response = json.loads(rv.data)
            expected_schema_fields = {
    ...
    def test_group_schema(self):
        ...
        test group schema
        """
        url = self.get_url_prefix() + '/groups/schema'
        with self.app.test_client() as client:
            rv = client.get(url)
            response = json.loads(rv.data)
            expected_schema_fields = {
    ...
    def test_node_schema(self):
        ...
        test node schema
        """
        url = self.get_url_prefix() + '/nodes/schema'
        with self.app.test_client() as client:
            rv = client.get(url)
            response = json.loads(rv.data)
            expected_schema_fields = {
    ...
    def test_calculation_schema(self):
        ...
        test computer schema
        """
        url = self.get_url_prefix() + '/calculations/schema'
        with self.app.test_client() as client:
            rv = client.get(url)
            response = json.loads(rv.data)
            expected_schema_fields = {
