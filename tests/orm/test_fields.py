...
def test_all_entity_fields(entity_cls, data_regression):
    data_regression.check({key: repr(value) for key, value in entity_cls.fields._dict.items()},
...
def test_all_node_fields(group, name, data_regression):
    ...
    node_cls = tuple(EPS.select(group=group, name=name))[0].load()
    data_regression.check({key: repr(value) for key, value in node_cls.fields._dict.items()},
...
def test_query_new_class(clear_database_before_test, monkeypatch):
    ...
    result = QueryBuilder().append(
        NewNode,
        tag='node',
        project=[NewNode.fields.key1, NewNode.fields.key2, NewNode.fields.key3],
        filters={
            NewNode.fields.key2: 2
        }
    ).order_by({
        'node': NewNode.fields.ctime
    }).all()
