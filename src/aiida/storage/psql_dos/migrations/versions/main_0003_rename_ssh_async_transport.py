...
The legacy paramiko-based ``core.ssh`` transport plugin was removed in v3.0, and the asynchronous
plugin (formerly ``core.ssh_async``) took over its entry point name. Consequently:
...
* computers configured with the legacy ``core.ssh`` keep their ``transport_type`` as is, and are now
  served by the asynchronous plugin, which honors the connection parameters that the legacy plugin
  had stored in ``db_dbauthinfo.auth_params`` (username, port, key_filename, proxy_jump, ...). Those
  parameters are left untouched by this migration.
