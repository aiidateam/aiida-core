...
def test_plugin_list(run_cli_command):
    ...
    for options in [[], ['-v', 'INFO']]:
        result = run_cli_command(plugin_list, ['aiida.calculations'] + options)
        assert result.exception is None
        assert 'arithmetic.add' in result.stdout, result.stdout

    result = run_cli_command(plugin_list, ['aiida.calculations', '-v', 'WARNING'])
    assert result.exception is None
    assert 'arithmetic.add' not in result.stdout, result.stdout

    result = run_cli_command(plugin_list, ['aiida.calculations', '-v', 'BOGUS'], raises=True)
