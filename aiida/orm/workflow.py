...
class Workflow(object):
    ...
    implementations, and provides all the methods necessary to interact with
    the database.
    
    The typical use case are workflow stored in the aiida.workflow packages,
    that are initiated either by the user in the shell or by some scripts,
    and that are monitored by the aiida daemon.
    
    Worflow can have steps, and each step must contain some calculations to be
    executed. At the end of the step's calculations the workflow is reloaded in
    memory and the next methids is called.
    """
...
def list_workflows(expired=False):
    ...
    for w in w_list:
        print ""
        print "Workflow {0}.{1} [uuid: {3}] started at {2} is {4}"
                .format(w.module,w.module_class, w.time, w.uuid, w.status)
