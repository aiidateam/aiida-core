...
from aiida.backends.sqlalchemy.migrations.utils.calc_state import STATE_MAPPING, StateMapping
...
def test_legacy_jobcalcstate(perform_migrations: PsqlDostoreMigrator):
    ...
    with perform_migrations.session() as session:
        for node_id, mapping in nodes.items():
            attributes = session.query(node_model).get(node_id).attributes
