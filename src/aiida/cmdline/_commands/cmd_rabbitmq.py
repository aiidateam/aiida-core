...
from aiida.cmdline._commands.cmd_devel import verdi_devel
...
def with_client(ctx, wrapped, _, args, kwargs):
    """Decorate a function injecting a :class:`aiida.brokers.rabbitmq._client.RabbitmqManagementClient`."""
    from aiida.brokers.rabbitmq._client import RabbitmqManagementClient
...
def cmd_tasks_list(broker):
    ...
    running.
    """
    from aiida.engine.processes._control import get_process_tasks
...
def cmd_tasks_revive(processes, force):
    ...
    """
    from aiida.engine.processes._control import revive_processes
