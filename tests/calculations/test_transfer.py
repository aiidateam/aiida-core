...
def test_integration_transfer(aiida_localhost, tmp_path):
    ...
    assert sorted(output_retrieved.ctx.repository.list_object_names()) == sorted(['file_local.txt', 'file_remote.txt'])
    assert output_retrieved.ctx.repository.get_object_content('file_local.txt') == content_local
    assert output_retrieved.ctx.repository.get_object_content('file_remote.txt') == content_remote
