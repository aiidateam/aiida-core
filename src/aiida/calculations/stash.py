...
def validate_source_node(source_node: Any, _: Any) -> Optional[str]:
    ...
    if creator is not None and not creator.is_sealed:
        return (
            'The source_node is still in writing by a running calculation. '
            'You cannot stash files from a running CalcJob. '
            'Because it might skip files that are not yet written to the remote folder. '
            'Either use the direct stashing method (via metadata.options.stash), '
            'or use a WorkChain/WorkGraph to ensure proper sequencing.'
        )
