...
def load_dbenv(profile=None, *args, **kwargs):
    if PROFILE.database_backend == BACKEND_SQLA:
        ...
        raise ConfigurationError("Invalid PROFILE.database_backend: {}".format(
...
def _load_dbenv_noschemacheck(profile=None, *args, **kwargs):
    # This is going to set global variables in settings, including PROFILE.database_backend
    ...
    if PROFILE.database_backend == BACKEND_SQLA:
        ...
        raise ConfigurationError("Invalid PROFILE.database_backend: {}".format(PROFILE.database_backend))

    return to_return


def get_global_setting(key):
    if PROFILE.database_backend == BACKEND_DJANGO:
        from aiida.backends.djsite.globalsettings import get_global_setting
    elif PROFILE.database_backend == BACKEND_SQLA:
        from aiida.backends.sqlalchemy.globalsettings import get_global_setting
    else:
        raise Exception("unknown backend {}".format(PROFILE.database_backend))
