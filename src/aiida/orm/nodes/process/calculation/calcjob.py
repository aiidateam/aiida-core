...
class CalcJobNode(CalculationNode):
    ...
    class Model(CalculationNode.Model):
        scheduler_state: Optional[str] = MetadataField(description='The state of the scheduler')
        state: Optional[str] = MetadataField(description='The active state of the calculation job')
        remote_workdir: Optional[str] = MetadataField(description='The path to the remote (on cluster) scratch folder')
        job_id: Optional[str] = MetadataField(description='The scheduler job id')
        scheduler_lastchecktime: Optional[str] = MetadataField(
            description='The last time the scheduler was checked, in isoformat'
        )
        last_job_info: Optional[str] = MetadataField(description='The last job info returned by the scheduler')
        detailed_job_info: Optional[dict] = MetadataField(description='The detailed job info returned by the scheduler')
        ...
        imported: Optional[bool] = MetadataField(description='Whether the node has been migrated')
