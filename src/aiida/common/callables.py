...
def resolves_here(value: t.Any) -> bool:
    ...
    Importability is a fact about one interpreter, so this is :func:`resolves_in` asked about the one running now.
    Whoever reads an object back somewhere else has to ask that one instead.
...
def resolves_in(value: t.Any, search_paths: Sequence[str]) -> bool:
    ...
    :func:`resolves_here` answers the same question for the interpreter running now, which is the wrong one
    whenever the payload is read back somewhere else: a module in a directory added to ``sys.path`` after the daemon
    was started imports here and not there.

    :param value: The object to check.
    :param search_paths: The ``sys.path`` of the interpreter that would do the importing.
    :returns: ``True`` if that interpreter would find the very object this name refers to here.
    """
    return resolves_here(value) and module_resolves_in(value.__module__, search_paths)
...
def fingerprint(value: t.Any) -> str:
    ...
    The reference form is hashed, so a callable that can be imported is identified by its name, which no change of
    pickler can alter.
