...
class SinglefileData(Data):
    ...
    def open(self, path=None, mode='r'):
        ...
        with self.ctx.repository.open(path, mode=mode) as handle:
            yield handle
    ...
    def set_file(self, file, filename=None):
        ...
        existing_object_names = self.ctx.repository.list_object_names()
        ...
        if is_filelike:
            self.ctx.repository.put_object_from_filelike(file, key)
        else:
            self.ctx.repository.put_object_from_file(file, key)
        ...
        for existing_key in existing_object_names:
            self.ctx.repository.delete_object(existing_key)
    ...
    def _validate(self):
        ...
        objects = self.ctx.repository.list_object_names()
