...
class ProfileDumper(BaseDumper):
    ...
    def __init__(
        ...
        self.only_top_level_workflows = (
            self.profile_dump_config.only_top_level_workflows
        )
    ...
    def _dump_processes_not_in_any_group(self) -> None:
        ...
        if (
            ...
            and not no_group_dumper.processes_to_dump.is_empty
        ):
    ...
    def _get_no_group_processes(self) -> list[str]:
        ...
        nodes_in_groups: list[str] = [
            node.uuid for group in profile_groups for node in group.nodes
        ]
        ...
        process_nodes: list[str] = [
            profile_node
            for profile_node in profile_processes
            if profile_node not in nodes_in_groups
        ]
        process_nodes = filter_nodes_last_dump_time(
    ...
    def dump_processes(self):
        ...
        if not self.groups:
            ...
            self._dump_processes_per_group(
                groups=[g for g in self.groups if g is not None]
            )
    ...
    def _get_processes_to_delete(self) -> list[str]:
        ...
        # Cannot use QB here because, when node deleted, it's not in the DB anymore
        dumped_uuids = set(
            list(log.calculations.entries.keys()) + list(log.workflows.entries.keys())
        )
        ...
        profile_uuids = cast(
            ...
            set(
                orm.QueryBuilder()
                .append(orm.ProcessNode, project=["uuid"])
                .all(flat=True)
            ),
        )
    ...
    def delete_processes(self):
        ...
        for to_delete_uuid in to_delete_processes:
            delete_missing_node_dir(
                dump_logger=self.dump_logger, to_delete_uuid=to_delete_uuid
            )
    ...
    def update_groups(self) -> list[dict[str, Path]]:
        ...
        new_mapping: dict[str, Path] = dict(
            zip(
                dumped_group_uuids,
                [
                    self.dump_parent_path
                    / "groups"
                    / get_group_subpath(orm.load_group(g))
                    for g in dumped_group_uuids
                ],
            )
        )
