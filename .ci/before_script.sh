...
then
    ...
    then
        ...
        cat ${TRAVIS_BUILD_DIR}/.ci/computer-torquessh-setup-input.txt | verdi -p $TEST_AIIDA_BACKEND computer setup --non-interactive --label=torquessh --hostname=localhost --enabled --transport=ssh --scheduler=torque --mpiprocs-per-machine=1 --prepend-text="" --append-text=""
