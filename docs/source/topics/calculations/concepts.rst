...
These code nodes are instances of the :py:class:`~aiida.orm.Code` class, which is a sub-class of :py:class:`~aiida.orm.nodes.data.data.Data`.
...
Since the :py:class:`~aiida.orm.Code` that is used as an input for the calculation job, which is configured for a specific :py:class:`~aiida.orm.computers.Computer`, the engine knows exactly how to execute all these tasks.
