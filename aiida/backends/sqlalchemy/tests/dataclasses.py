...
if has_pymatgen():
    from pymatgen.io.smartio import read_mol, read_structure


class TestCalcStatus(SqlAlchemyTests):
    """
    ...
    def test_state(self):
        ...
        # Use the SqlAlchemyTests test computer
...
class TestSinglefileData(SqlAlchemyTests):
    """
...
class TestCifData(SqlAlchemyTests):
    """
    ...
    def test_pycifrw_from_datablocks(self):
        ...
        for line in lines:
            if not re.search('^#', line):
                # XXX we change single space line into an empty char because
                # some editor are removing trailing whitespace on save.
                if line == ' ':
                    line = ''
...
class TestKindValidSymbols(SqlAlchemyTests):
    """
...
class TestSiteValidWeights(SqlAlchemyTests):
    """
...
class TestKindTestGeneral(SqlAlchemyTests):
    """
...
class TestKindTestMasses(SqlAlchemyTests):
    """
...
class TestStructureDataInit(SqlAlchemyTests):
    """
...
class TestStructureData(SqlAlchemyTests):
    """
    Tests the creation of StructureData objects (cell and pbc).
    """
    ...
    def test_get_cif(self):
        ...
        # Exception thrown if ase can't be found
            ...
            return
        # XXX remove trailing space
        cif_prepared = c._prepare_cif()
        cif_prepared = "\n".join(
            map(lambda _: re.sub(r"( )+$", "", _), cif_prepared.split("\n"))
        )
        s = """#\#CIF1.1
##########################################################################
...
"""
        self.assertEquals(cif_prepared, s)


class TestStructureDataLock(SqlAlchemyTests):
    """
...
class TestStructureDataReload(SqlAlchemyTests):
    """
...
class TestStructureDataFromAse(SqlAlchemyTests):
    """
...
class TestStructureDataFromPymatgen(SqlAlchemyTests):
    """
...
class TestPymatgenFromStructureData(SqlAlchemyTests):
    """
...
class TestArrayData(SqlAlchemyTests):
    """
...
class TestTrajectoryData(SqlAlchemyTests):
    """
...
class TestKpointsData(SqlAlchemyTests):
    """
    Tests the TrajectoryData objects.
    """
...
class TestData(SqlAlchemyTests):
    """
