...
class TestDbLogMigrationRecordCleaning:
    ...
    def setup_db(self, perform_migrations: Migrator):  # pylint: disable=too-many-locals,too-many-statements
        ...
        # starting revision
        perform_migrations.migrate_down('7ca08c391c49')  # 7ca08c391c49_calc_job_option_attribute_keys
        ...
        with perform_migrations.session() as session:
            ...
            serialized_unknown_exp_logs = log_migration.get_serialized_unknown_entity_logs(session)
            # Getting their number
            unknown_exp_logs_number = log_migration.get_unknown_entity_log_number(session)
            ...
            # provides them) - this should coincide to the above
            serialized_leg_wf_exp_logs = log_migration.get_serialized_legacy_workflow_logs(session)
            eg_wf_exp_logs_number = log_migration.get_legacy_workflow_log_number(session)
            ...
            # provides them) - this should coincide to the above
            serialized_logs_exp_no_node = log_migration.get_serialized_logs_with_no_nodes(session)
            logs_no_node_number = log_migration.get_logs_with_no_nodes_number(session)
            self.to_check['NoNode'] = (serialized_logs_no_node, serialized_logs_exp_no_node, logs_no_node_number)

        # migrate up
        perform_migrations.migrate_up('041a79fc615f')  # 041a79fc615f_dblog_cleaning
...
def test_backward_migration(perform_migrations: Migrator):
    ...
    with perform_migrations.session() as session:
        ...
        to_check = {
            log_1.id: (log_1.dbnode_id, calc_1.type),
            log_2.id: (log_2.dbnode_id, calc_2.type)
        }

    # migrate down
    perform_migrations.migrate_down('7ca08c391c49')  # e72ad251bcdb_dbgroup_class_change_type_string_values

    # perform some checks
    DbLog = perform_migrations.get_current_table('db_dblog')  # pylint: disable=invalid-name
    with perform_migrations.session() as session:

        for log_pk, to_check_value in to_check.items():
            log_entry = session.query(DbLog).filter(DbLog.id == log_pk).one()
            log_dbnode_id, node_type = to_check_value

            assert log_dbnode_id == log_entry.objpk, (
                f'The dbnode_id ({log_dbnode_id}) of the 0024 schema version '
                f'should be identical to the objpk ({log_entry.objpk}) of the 0023 schema version.')
            assert node_type == log_entry.objname, (
                f'The type ({node_type}) of the linked node of the 0024 schema version should be identical to the '
                f'objname ({log_entry.objname}) of the 0023 schema version.')
            assert log_dbnode_id == log_entry.metadata['objpk'], (
                f'The dbnode_id ({log_dbnode_id}) of the 0024 schema version should be identical to the '
                f'objpk ({log_entry.metadata["objpk"]}) of the 0023 schema version stored in the metadata.')
