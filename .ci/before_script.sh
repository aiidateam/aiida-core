...
then
    ...
    then
        ...
        verdi -p $TEST_AIIDA_BACKEND computer configure ssh torquessh --non-interactive --label=jenkins --port=22 --key-filename=~/.ssh/id_rsa --timeout=60 --compress --gss-host=localhost --load-system-host-keys --key-policy=RejectPolicy
