        ...
        for requirement_abstract in requirements_abstract:
            for requirement_concrete in requirements_concrete:
                if '@' in str(requirement_concrete):
                    # For versions that are using `@` specifier as for git repos,
                    # since `@` is not listed as a valid `specifier` in class `Specifier`.
                    version = '@'.join(str(requirement_concrete).split('@')[1:])
