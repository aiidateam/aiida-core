...
* **Compression:** By default, the export file is compressed using ``gzip``, creating a tar ball (``tar.gz``).
  Use the command line options to compress using ``zip`` instead or create an uncompressed ``zip`` archive.


Import
++++++

Use ``verdi import`` to import an AiiDA export archive generated by ``verdi export create``, either from a local file or from a URL.
